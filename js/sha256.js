 ...
 *  饮库 —— SHA-256（纯 JS，无依赖）
 *  ------------------------------------------------------------
 *  为什么自己写一个而不用浏览器的 crypto.subtle：
 *  crypto.subtle 只在「安全上下文」里可用，本地双击打开（file://）时
 *  各家浏览器行为不一致。这个自带的版本在哪都能跑。
 *  用途只有一个：把审核台的口令变成摘要，仓库里不出现明文。
 * ============================================================ */
(function (global) {
  ...
  /** 把 JS 字符串按 UTF-8 拆成字节数组（中文口令也能正确处理） */
  function utf8Bytes(str) {
    ...
    for (var i = 0; i < str.length; i++) {
      ...
      if (c < 0x80) {
        ...
        out.push(0xf0 | (cc >> 18), 0x80 | ((cc >> 12) & 63), 0x80 | ((cc >> 6) & 63), 0x80 | (cc & 63));
      }
    }
    return out;
  }

  global.sha256Hex = function (str) {
  ...
  if (typeof module !== 'undefined' && module.exports) module.exports = global.sha256Hex;
